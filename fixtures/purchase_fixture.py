...
@pytest.fixture
def purchase_fixture(api, configs, middleware_fixture):
    ...
    data['token'] = "1111"
    # Отправка успешного запроса
    request_api = api.post_login_confirmation(post_data=json.dumps(data))
    assert request_api.status_code == 200
    resp = json.loads(request_api.text)
    # Проверка ответа на зарос
    ...
    # Отправка запроса для получения кол-ва рассрочек
    req_installment = api.installment_get(accessToken)
    # Проверка кода на отправленный запрос
    assert req_installment.status_code == 200
    resp = json.loads(req_installment.text)
    # Проверка успешности пиршедших данных
    assert resp['success']== True and resp['errorMessage'] == None
    installments = resp['result']['installments']
    apis = Merchant(configs['API_endpoints']['merchantsapi'])
        # Цикл для провеки кол-ва рассрочек
    for install in range(3):
            ...
            request_create_invoice = apis.create_invoice(api_key,post=json.dumps(data_invoice))
